...
- `project-scope-control.md` supports title-first thesis projects: the initial title is treated as a draft hypothesis, then reviewed at stages 1, 3, 4, 8, and 10 for title survival, causal availability, node/structure definition, and downgrade/rename routes.
...
- Stage 10 writing is split into three gates: **10A writing blueprint and first draft** locks the chapter structure, section titles, figure/table placement, claim-evidence responsibilities, and citation gaps before long-form prose; **10B-0 school-template style lock** maps the Chengdu University of Technology master's thesis template into executable DOCX constraints before numbering begins; **10B polish and manuscript integration** handles final section logic, hedging, figure captions, Word/WPS cross-references, DOCX production, Zotero/BibTeX insertion, and rendered PDF checks after the draft exists. Do not start final figure numbering before the正文初稿 and school-template style map are stable.
