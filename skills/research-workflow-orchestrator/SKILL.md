...
7. Experiment recording and result scan: `$jupyter-notebook`, `$research-results-analysis`, `$research-autoresearch-loop`, Spreadsheets for reviewable exports, and Build Web Data Visualization for result-chart choice and QA; create `experiment-reports/EXP-*.md` and update `benchmark-report-schema.md` for formal baseline comparisons; update `material-passport.md` and data availability when result artifacts become evidence.
