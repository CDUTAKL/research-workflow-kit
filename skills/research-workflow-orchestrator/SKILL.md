...
- Title-first thesis control: when the project starts from a fixed or tentative title, use `project-scope-control.md` to track title survival, causal availability, node/structure definition, and downgrade/rename policy across stages 1, 3, 4, 8, and 10.
...
- Figures, tables, diagrams, captions, Nature-style figure audit, and model-architecture rendering: use `$research-paper-figures`; for thesis route diagrams, method architecture diagrams, mechanism diagrams, workflow diagrams, and system-structure diagrams, prefer academic PPT templates followed by PPTX-safe editing, thumbnail review, WPS/PowerPoint manual polish, and final export. Image Gen is optional inspiration only, not the default mother draft. Use draw.io as a fallback when PPT templates are unsuitable or an editable vector source is required. On Windows with Microsoft Visio, optionally use the Visio JSON-plan route inspired by `codex-visio-replica-workflow`. Use Build Web Data Visualization for complex chart selection and dashboard/evidence graph design QA.
...
- Project overview: use `workflow-dashboard.md`, `project-scope-control.md`, `console-file-index.md`, `weekly-review.md`, the legacy-compatible `daily-workflow-entry.md`, `scripts/research_workflow_doctor.py`, `scripts/audit_project_scope.py`, `scripts/update_daily_workflow.py`, `scripts/update_weekly_review.py`, `scripts/export_evidence_graph.py`, `scripts/dashboard_control_server.py`, `scripts/edit_workflow_record.py`, and `dashboard-web/` when the user needs a one-page status view, title-scope control, file-layer guidance, weekly convergence, current research workspace, health check, focused evidence chain, experiment comparison, or local web dashboard with refresh/open/export/edit actions.
...
- Defense slides: use Presentations for PPTX creation/editing; use PPT template figures or draw.io exports for structured diagrams; use Figma or BioRender only as optional visual refinement tools when available. Canva is optional only if explicitly available and useful.
- PPT template editing / PPTX-safe editing is the default stage 9 route for method overview, technical route, workflow, mechanism, and system-structure figures when a suitable academic template exists. The workflow is: choose template -> fill thesis-specific Chinese labels -> inspect thumbnails -> apply safe object/XML edits only when needed -> WPS/PowerPoint manual polish -> export SVG/PDF/PNG/PPTX.
- Image Gen Skill is no longer mandatory for stage 9 method/architecture/route figures. It may be used only as optional inspiration when no suitable template/reference exists; its output is checked for content accuracy and must not be treated as final manuscript evidence.
- draw.io / draw.io MCP is a fallback stage 9 formal redraw route for model architecture, evidence graph, and system diagrams when PPT templates are unsuitable or editable vector structure is required. Export SVG/PDF/PNG for thesis use and package into PPTX for defense slides when needed.
...
| `project-scope-control.md` | title survival, causal availability, node/structure definition, downgrade/rename policy |
...
9. Figure and table planning: `$research-paper-figures`, with `figure-audit-standard.md` for Nature-derived claim-first figure QA, Build Web Data Visualization for chart/dashboard/evidence graph design QA, and Product Design for advisor-facing readability and hierarchy review. For route diagrams, method architecture diagrams, mechanism diagrams, workflow diagrams, and system-structure figures, use the PPT-template route first: figure contract -> select academic PPT template -> fill thesis-specific Chinese labels -> inspect thumbnails -> apply PPTX-safe edits -> WPS/PowerPoint manual polish -> SVG/PDF/PNG/PPTX export -> figure audit. Image Gen is optional inspiration only and no longer the default mother draft. Use draw.io when PPT templates are unsuitable or editable vector structure is required. Windows route: reference image -> Visio JSON plan -> `.vsdx` -> EMF/PDF/PNG export -> copy artifacts back -> audit. Use Python or the Nature-style renderer for data-backed plots, and Spreadsheets for manuscript tables.
10. Paper writing and polishing: split this stage into two gates so writing does not start from premature figure numbering.
    - **10A thesis writing blueprint and first draft**: use `$research-paper-plan` to lock the chapter structure, section titles, writing order, figure/table placement plan, and claim-evidence responsibilities; use `$research-results-analysis` to turn frozen EXP-103..EXP-106 evidence into section-level result narratives; use `$research-paper-writing` to draft method, experiment, result analysis, related work, introduction, abstract, and conclusion in that order unless the user supplies another template; use `$research-literature-review`, `$semanticscholar-skill`, Zotero, Scite, and `$pdf` to fill citation gaps through `section-citation-map.md`, `citation-provenance.md`, and source-grounded readings; use Spreadsheets for reviewable result/citation/claim tables. Main records: `writing-outline.md`, `claim-evidence-map.md`, `section-citation-map.md`, `result-narrative-map.md` when present, `figure-table-placement.md` when present, and `nature-style-writing-checklist.md`.
    - **10B thesis polish and manuscript integration**: after the section draft exists, use `$research-paper-writing` for section logic, hedging, sentence clarity, unsupported-claim removal, and Chinese thesis tone; use `$research-paper-figures` to verify captions and figure/text alignment; use Documents for `.docx` production only after the prose structure is stable; use Zotero/BibTeX export for citation insertion; use `$pdf` for rendered checks. Final numbering, Word/WPS cross-references, figure captions, and DOCX layout belong here, not before the first full draft.
