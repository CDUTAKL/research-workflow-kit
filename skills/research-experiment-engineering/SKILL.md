...
- Use local runs for debugging and smoke tests; use AutoDL or another cloud GPU for formal training only when the user provides access and asks for remote execution.
...
7. Choose execution target: `local` for smoke tests/debugging, `cloud_autodl` for formal training, or another cloud target when specified.
...
- execution target plan: local smoke test, AutoDL formal training, or both
