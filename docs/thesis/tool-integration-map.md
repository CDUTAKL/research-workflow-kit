...
| React/Vite dashboard | 1-12 | local daily workspace, workflow tabs, interactive evidence graph, section citation heatmap, flow editor, final artifact review controls | `dashboard-web/` | Run fixed local URL service or `pnpm run prepare:data`, then `pnpm run dev`; source records stay in Markdown/TSV/JSON |
...
| nature-academic-search source | 2, 10 | multi-source search routing, DOI/PMID/arXiv/S2 metadata checks, query provenance | `academic-search-policy.md`, `section-citation-map.md`, `citation-provenance.md` | Use as a search discipline layer; do not auto-download or cite unverified papers |
...
| `cloud_autodl` | 6-8 optional | stronger fallback GPU target with auto-save and auto-shutdown | `experiment-runbook.md`, `reproducibility-checklist.md`, `experiment-registry.md` | User creates the instance manually; use SSH templates to save logs, exit code, environment snapshot, run summary, checksums, archive outputs, and run `/usr/bin/shutdown` after completion |
| Mac document production | 11 | WPS-compatible DOCX main production, PDF export candidate, PPTX candidate, artifact manifest/checksum | `final-artifact-manifest.md`, `final-audit.md`, `defense-prep.md` | Mac is now the primary document production environment |
| Windows compatibility review | 12 | Word/WPS/PowerPoint compatibility checks, final submission checks, defense playback testing | `final-audit.md`, `defense-prep.md`, `final-artifact-manifest.md` | Windows laptop is the final verification environment, not the main production environment |
...
| nature-writing / polishing source | 10-12 | section logic, gap statement, overclaim checks, reviewer-style thesis precheck | `nature-style-writing-checklist.md`, `final-audit.md` | Use after evidence/citations are stable; do not strengthen unsupported claims |
...
| 2. Literature discovery, Zotero intake, and review | `$semanticscholar-skill`, `$research-literature-review`, Zotero, Scite, nature-academic-search source | `academic-search-policy.md` for source routing; Zotero for local library/metadata/PDF/citation keys; `zotero-literature-hub.md` for inventory/policy snapshot; `zotero-screening-loop.md` for candidate screening; `zotero-collection-coverage.md` for section coverage; `section-citation-map.md` for section-level citation matching; Scite for support/contrast/mention checks |
...
| 6. Experiment run and monitoring | `$research-experiment-engineering`, `$research-autoresearch-loop` | GitHub commit trace, `local_mac` CPU-only smoke tests, `remote_desktop_4060` primary GPU runs and full-artifact storage, `cloud_autodl` fallback with auto-save/auto-shutdown templates, macOS Terminal / VS Code SSH / `ssh` / `scp` / `rsync`, Notion progress; Codex Security for remote script changes |
...
| 9. Figure and table planning | `$research-paper-figures`, Image Gen visual reference, draw.io formal redraw, optional Windows Visio route, Python/Nature-style renderer, nature-figure source, Build Web Data Visualization | Image Gen for attractive reference; Mac draw.io for structured diagrams; Windows Visio for editable `.vsdx`; Python for data-backed plots; `figure-style-qa.md` for advisor/final figures; Product Design for advisor-facing visual review; Presentations for PPTX packaging |
| 10. Paper writing | `$research-paper-writing`, nature-writing / polishing source, Documents | Zotero for citations and BibTeX export; check `nature-style-writing-checklist.md`, `section-citation-map.md`, `citation-provenance.md`, and `zotero-literature-hub.md` before polishing |
| 11. Mac DOCX / PDF / PPTX main production | WPS, Documents, `$research-data-availability`, LaTeX doctor before optional LaTeX compile, PDF, Presentations | Complete DOCX/PDF/PPTX main artifacts on Mac, check evidence/citations, register final artifacts, and package for Windows compatibility review |
| 12. Windows compatibility review and final submission preparation | `$research-final-audit`, `$research-data-availability`, Windows Word/WPS/PowerPoint, Presentations, draw.io exports, nature-paper2ppt source | Windows laptop opens/verifies Mac-produced DOCX/PDF/PPTX, checks layout/export/playback compatibility, and runs final audit; Notion tasks, Figma/BioRender optional, Canva optional only if available |
